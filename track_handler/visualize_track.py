...
def create_cube(size, color, position):
	...
	position_adapted = [position[0] - (size[2] // 2), position[1] - (size[2] // 2), position[2] - (size[2] // 2)]
...
def get_information_from_track_layout_image(path):
...
def merge_start_trail_end(start, trail_composition, end):
	...
	result.append(["START", start[0], start[1]])
...
def get_track_layout_markers(image_layout_path, triangles_list):
	print("==> layout image path : ", image_layout_path)

	trackers_information = get_information_from_track_layout_image(image_layout_path)
	print("==> Markers in 2D : ", len(trackers_information))
...
def main():
	...
	track_layout_markers = get_track_layout_markers(image_layout_path, triangles_set)
